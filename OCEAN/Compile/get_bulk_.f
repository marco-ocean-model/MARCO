  ...
  99  may_day_flag=2
     ...
     &                                           .not.SOUTH_INTER)) then
            ...
            if ((istr.eq.1 .and. .not.WEST_INTER) .and. (jend.eq.Mmmpi 
     &                                     .and. .not.NORTH_INTER)) then
              wspd(Istr-1,Jend+1)=wspd(Istr,Jend)
            endif
            if ((iend.eq.Lmmpi .and. .not.EAST_INTER) .and. 
     &                           (jstr.eq.1.and. .not.SOUTH_INTER)) then
              wspd(Iend+1,Jstr-1)=wspd(Iend,Jstr)
            endif
            if ((iend.eq.Lmmpi .and. .not.EAST_INTER) .and. 
     &                      (jend.eq.Mmmpi .and. .not.NORTH_INTER)) then

...
!$acc kernels if(compute_on_device) default(present) async(1)
     ...
     &                      *rmask(0,j)
      ...
      if ((iend.eq.Lmmpi .and. .not.EAST_INTER)) then
        do j=JstrV-1,Jend
          cx=dtfast*pm(Iend,j)*
     &                      sqrt(g*max(0.D0,h(Iend,j)+zeta(Iend,j,
     &                                                           kstp)))
     ...
     &                      *rmask(i,Jend+1)
      ...
      if ((jstr.eq.1.and. .not.SOUTH_INTER) .and. (istr.eq.1 .and. 
     &                                            .not.WEST_INTER)) then
        zeta(0,0,knew)=0.5D0*(zeta(1,0 ,knew)+zeta(0 ,1,knew))
     &                       *rmask(0,0)
      endif
      if ((jstr.eq.1.and. .not.SOUTH_INTER) .and. (iend.eq.Lmmpi .and. 
     &                                            .not.EAST_INTER)) then
        zeta(Iend+1,0,knew)=0.5D0*(zeta(Iend+1,1 
     &                                         ,knew)+zeta(Iend,0,knew))
     &                       *rmask(Iend+1,0)
      endif
      if ((jend.eq.Mmmpi .and. .not.NORTH_INTER) .and. (istr.eq.1 .and. 
     &                                            .not.WEST_INTER)) then
        zeta(0,Jend+1,knew)=0.5D0*(zeta(0,Jend,knew)+zeta(1 
     &                                                    ,Jend+1,knew))
     &                       *rmask(0,Jend+1)
      endif
      if ((jend.eq.Mmmpi .and. .not.NORTH_INTER) .and. (iend.eq.Lmmpi 
     &                                      .and. .not.EAST_INTER)) then

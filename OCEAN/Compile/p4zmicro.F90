...
CONTAINS
   ...
   SUBROUTINE p4z_micro( kt, knt, Kbb, Krhs )
      ...
      !$OMP PARALLEL DO &
      !$OMP PRIVATE(ji, jj, jk) &
      !$OMP SHARED(tr, xstep, tgfunc2, sized, nitrfac, rtrn, xprefn, &
      ...
      DO_3D( 0, 0, 0, 0, 1, jpk)
         trc3d(ji,jj,jkR,jp_grapoc) = zgrazing(ji,jj,jk) * 1.e+3 * rfact2r &
                                    * tmask(ji,jj,jk) !  grazing of phyto by microzoo
         trc3d(ji,jj,jkR,jp_mico2)  = zgrazing(ji,jj,jk) * ( 1.- epsher - unass ) &
          &                         * (-o2ut) * sigma1 * 1.e+3 * rfact2r &
                                    * tmask(ji,jj,jk) ! o2 consumption by Microzoo
      END_3D
      !$OMP END PARALLEL DO
#endif

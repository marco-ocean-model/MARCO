...
CONTAINS
   ...
   SUBROUTINE p4z_meso( kt, knt, Kbb, Kmm, Krhs )
      ...
      IF( l_dia_lprodz ) THEN
         ...
         !$OMP SHARED(zzligprod, tr, jplgw, Krhs, jpkm1)
      ...
      !
      !$OMP PARALLEL DO &
      !$OMP PRIVATE(ji, jj, jk, zcompam, zfact, zrespz, ztortz, zcompadi, &
      ...
      !$OMP SHARED(tr, nitrfac, tgfunc2, xstep, resrat2, xkmort, mzrat2, xpref2d, &
                    ...
                    xsigma2, xsigma2del, rtrn, feratm, epsher2, epsher2min, unass2, &
                    part2, prodcal, jpkm1)
      DO_3D( 0, 0, 0, 0, 1, jpkm1)
      ...
      IF (ln_dvm_meso) THEN
        ...
        ! --------------------------------------------------------------------
        !$OMP PARALLEL DO &
        !$OMP PRIVATE(ji, jj, jk, zmigreltime, zmigthick) &
      ...
      DO_3D( 0, 0, 0, 0, 1, jpk)
         trc3d(ji,jj,jkR,jp_grapoc2) = zgrazing2(ji,jj,jk) * 1.e+3 &
                                     * rfact2r * tmask(ji,jj,jk) !  grazing of phyto by mesozoo
         trc3d(ji,jj,jkR,jp_meso2)   = zgrazing2(ji,jj,jk) * ( 1. - epsher2 - unass2 ) &
            &                        * (-o2ut) * sigma2 * 1.e+3 * rfact2r &
                                     * tmask(ji,jj,jk) ! o2 consumption by Mesozoo
      END_3D
      !$OMP END PARALLEL DO
#endif
  ...
  !      CALL prt_ctl(tab4d_1=tr(:,:,:,:,Krhs), mask1=tmask, clinfo=ctrcnm)
   ...
   SUBROUTINE p4z_meso_depmig( Kbb, Kmm )
      ...
      ! ------------------------------------------------------------------------------------
      !$OMP PARALLEL DO &
      !$OMP PRIVATE(ji,jj,z1dep) &
      !$OMP SHARED(oxymoy,tempmoy,tr,ts,zdepmoy,rtrn)
      DO_2D( 0, 0, 0, 0 )
